...
#if 0
				for(i = 0;i < maxlen;i++)
					LOGE("readPort buf[%d]=0x%02x",i,readb[i]);
#endif
				return maxlen;
				//return SUCCESS;
			}
...
int readPkg(unsigned char* readb)
{
	int ret = -1;
	int len = 0;
	int nLen = 0;
	...
	ret = readPort(readb, 7);
	if(ret != 7)
	{
		LOGE("read frame head error");
		return -1;
	}
	nLen += ret;
	p = readb;
	if((*p != 0xAA) && (*(p++) != 0xAA) && (*(p++) != 0xAA) && (*(p++) != 0x96) && (*(p++) != 0x69))
	{
		LOGE("frame head is not 0xAAAAAA9669");
		return -2;
	}

	len = (readb[5] << 8) | readb[6];
	LOGI("len = %d",len);

	ret = readPort(readb+7,len);
	if(ret != len)
	{
		LOGE("read data fail ret = %d",ret);
		return -1;
	}
