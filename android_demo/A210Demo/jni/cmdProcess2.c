...
int sendMutliPkt(unsigned char cmd,unsigned char *data,unsigned int dataLen)
{
	...
	if(dataLen <= INFO_SIZE)
	{
		...
		reSend:
			...
			rc = pthread_cond_timedwait(&queue_cv, &queue_start,&outtime);
			LOGI("[%s][%d]:finish timedwait rc = %d\n",__func__,__LINE__,rc);
...
int Cmd_startIdentify(unsigned char *userId)
{
	int ret = 0;
	unsigned char rvData[525];
	unsigned short rvLen = 0;
...
int Cmd_startRegister(unsigned char *userId)
{
	int ret = 0;
	unsigned char rvData[525];
	unsigned short rvLen = 0;
...
int Cmd_getLocalUser(unsigned char *userId)
{
	int ret = 0;
	unsigned int dataLen = 0;
...
int Cmd_getModel(unsigned char *userId)
{
	int ret = 0;
	unsigned char rvData[525];
	unsigned int dataLen = 0;
